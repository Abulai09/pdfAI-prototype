...
def _process_halyk_pdf_once(
    ...
    result = bytes(raw)
    # xref перестраивается БЕЗУСЛОВНО (2026-08-10). Раньше здесь стояло
    # `if cumulative_offset != 0 or _newly_available_cids`, и это ломало файл.
    # `cumulative_offset` копит только сдвиги от замен в content-стримах, а
    # длину меняют и другие проходы (патч Bold-глифов, стирание минуса), плюс
    # сама сумма обращается в ноль, когда растущие и уменьшающиеся замены
    # взаимно гасятся, — при этом отдельные объекты уже разъехались.
    #
    # Опасность этой ветки была описана в комментарии на этом самом месте ещё
    # в прошлой сессии, но закрыт был лишь частный случай (`or
    # _newly_available_cids`). Замер 2026-08-10: `HALYKformat2 ×1.05` даёт
    # битый xref на 6 сидах из 60 (12, 14, 23, 29, 38, 49) — до 36 битых
    # offsets из 47, PyMuPDF пишет «expected object number» и «too many kids
    # in page tree». Это каждый десятый прогон на связке, а не экзотика.
    #
    # Чинить учётом всех источников сдвига значило бы обязать КАЖДУЮ будущую
    # правку длины не забыть про накопитель — ровно эта ошибка уже случилась
    # дважды. Безусловный вызов снимает класс целиком и ничего не стоит:
    # `_rebuild_xref_table` побайтово ИДЕМПОТЕНТНА на файле с уже верным xref
    # (проверено на 8 реальных файлах трёх форматов), поэтому лишний вызов не
    # может создать расхождение стиля.
...
def _dotnet_guid_v4(raw16: bytes) -> str:
    """16 байт → hex в раскладке `System.Guid.ToByteArray()` с нибблами UUIDv4.

    .NET сериализует первые три поля GUID little-endian, поэтому нибл версии,
    который в канонической записи RFC 4122 стоит на hex-позиции 12, здесь
    оказывается на 14-й, а нибл варианта остаётся на 16-й. Проставляем их
    прямо в этих позициях — так значение раскрывается в настоящий UUIDv4 при
    обратной перестановке, ровно как у всех шести файлов корпуса.
    """
    b = bytearray(raw16[:16])
    b[7] = (b[7] & 0x0F) | 0x40          # версия 4 — hex-позиция 14
    b[8] = (b[8] & 0x3F) | 0x80          # вариант RFC 4122 — hex-позиция 16
    return b.hex().upper()
...
def _regenerate_trailer_id(
    ...
    с содержимым и никогда не совпадает с оригиналом.

    **Форма значения важна не меньше самого факта пересчёта (найдено
    2026-08-09).** Первая версия брала обе половины из `hashlib.md5`, и
    признак не исчез, а сменился на противоположный: генератор пишет
    `System.Guid.NewGuid().ToByteArray()`, то есть настоящий UUIDv4, а
    MD5-дайджест равномерно случаен и нибблов версии/варианта не имеет вовсе.
    Замер на корпусе: форма UUIDv4 у 6 оригиналов из 6 и лишь у 1 результата
    из 25 (случайное попадание с вероятностью 1/64). Поэтому байты берутся
    по-прежнему детерминированно, но нибблы версии и варианта проставляются —
    в позициях раскладки .NET (первые три поля little-endian, из-за чего
    версия уезжает с 12-й hex-позиции на 14-ю, а вариант остаётся на 16-й).

    Равенство половин НЕ выдумывается, а копируется из обрабатываемого файла:
    у 5 файлов из 6 `ID[0] == ID[1]` (свежесобранный документ), а у
    `HALYKformat1` они различны и второй — уже не Guid, а хеш, потому что
    документ пересохраняли. Повторяем ту конвенцию, что была, — та же
    дисциплина «читать правило из самого файла», что у `_op_separators`.
    """
    ...
    halves_were_equal = m.group(2).upper() == m.group(4).upper()
    id0 = _dotnet_guid_v4(hashlib.md5(pdf_bytes + b"\x00").digest())
    # Вторая половина повторяет конвенцию файла: у свежесобранного документа
    # она равна первой, у пересохранённого — самостоятельное значение, и там
    # генератор пишет уже не Guid, а хеш, поэтому форму ему не навязываем.
    id1 = id0 if halves_were_equal else hashlib.md5(pdf_bytes + b"\x01").hexdigest().upper()
